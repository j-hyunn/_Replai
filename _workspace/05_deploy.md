...
- 2026-09-11 (4차) **D34 반영 — 무료 티어 실측이 모델 선택을 뒤집었습니다.** 1.2절·2.3절·3.1절·5절·6절·8절만 고쳤습니다.
  - **3.1절 표를 실측값으로 교체.** `gemini-3.1-flash-lite`만 쓸 수 있습니다(15 RPM / 250K TPM / **500 RPD**).
    `gemini-2.5-*` 계열은 RPD **20**, Pro 계열은 RPD **0**입니다. `[확인 필요]` 5행이 해소됐습니다.
  - **5개 역할 전부 단일 모델.** `AI_MODEL_*` 5종의 기본값이 같아졌습니다(1.2절).
  - **예약 기본값 26/4/3 → `34`/`0`/`0`.** `flash`·`pro`는 **휴면 버킷**이 됩니다 — DDL·CHECK·처리 순서는
    그대로 두고 예약량만 0입니다. `AI_RPD_LIMIT_FLASH`·`_PRO`는 **실측하지 않고 비워 둡니다**.
  - **하루 체험 정원 = 12세션/일**(서비스 전체 합계). 정원을 결정하는 것은 이제 `pro`가 아니라
    **`flash_lite`의 RPD**입니다(3.1절 #3).
  - **"Evaluator를 `flash`로 내린다"는 첫 번째 조정 레버가 삭제됐습니다** — 내릴 곳이 없습니다.
    대체 레버 4단(BYOK 유도 → 미실측 최신 Flash 실측 → 재시도 여유 축소 → 동시 세션 상한)을 3.1.1절에 적었습니다.
  - **⚠️ 미해결:** "한도 **3종** 미설정 시 fail-open" 규칙이 휴면 버킷과 충돌합니다(1.2절 경고 상자).
...
| **`AI_RPD_LIMIT_FLASH_LITE`** | 측정한 `gemini-3.1-flash-lite`의 RPD **원값**. 기본값 **없음**(**2026-09-11 실측 `500`** 을 넣습니다). **유일한 활성 버킷이라 이 값 하나가 체험 정원을 결정합니다** | 〃 (`limit_calls` 계산) | 없음. 단 **체험 정원이 그대로 드러나므로 서버 전용** |
| **`AI_RPD_LIMIT_FLASH`** | **휴면 버킷**(D34). 예약량이 0이라 참조되지 않습니다 — **실측할 필요 없이 비워 둡니다** | 〃 | 〃 |
| **`AI_RPD_LIMIT_PRO`** | **휴면 버킷**(D34). `gemini-2.5-pro`·`gemini-3.1-pro`는 무료 티어 RPD가 **0**이라 넣을 값 자체가 없습니다 — 비워 둡니다 | 〃 | 〃 |
...
**⚠️ 한도가 미설정이면 게이트는 열린 채로 동작합니다 (fail-open — 확정 동작).**

> **⚠️ D34가 만든 미해결 모순 — 구현 전에 반드시 확정해야 합니다.**
> `02_ai_architecture.md` 8.3.8절의 원문은 **"한도 3종이 하나라도 설정되지 않았으면 fail-open"** 입니다.
> 그런데 D34 이후 `AI_RPD_LIMIT_FLASH`·`AI_RPD_LIMIT_PRO`는 **의도적으로 비워 두는 값**이므로,
> 그 규칙을 글자 그대로 구현하면 **게이트가 영구히 fail-open이 되어 D27이 통째로 죽습니다.**
> 8.3.5절이 "요청량 0인 버킷은 원장 행도 만들지 않고 건너뛴다"고 정한 것과 맞추면
> **검사 대상은 "예약량이 0보다 큰 버킷의 한도"뿐**이어야 하고, 지금 그것은 `FLASH_LITE` 하나입니다.
> 이 절은 후자로 읽는 것이 설계 의도에 맞다고 보지만, **8.3.8절 원문이 갱신되기 전까지는 미확정**입니다.
> **`ai-interview-architect`가 8.3.8절 문구를 확정해야 하며, 그 전에는 게이트를 구현하지 않습니다.**
...
**배포 없이 모델을 바꿀 수 있어야** 하기 때문입니다. 다만 **D34 이후 그 레버의 방향이 바뀌었습니다.**

> **⚠️ "Evaluator를 `flash`로 내린다"는 첫 번째 레버는 더 이상 존재하지 않습니다.**
> 2026-09-11 실측(`02_ai_architecture.md` 4.2.0절)에서 `gemini-3-flash`·`gemini-2.5-flash`의 RPD가
> **20**, `gemini-2.5-pro`·`gemini-3.1-pro`의 RPD가 **0**으로 확인됐습니다.
> **내릴 곳도 올릴 곳도 없습니다** — 5개 역할이 이미 유일하게 쓸 수 있는 모델에 모여 있습니다.

**남아 있는 실제 레버는 등급 조정이 아니라 아래 3가지입니다.**

```
1) 미실측 최신 Flash 계열을 실측해 더 큰 RPD를 찾는다  ← 정원을 늘리는 유일한 레버
   대상 후보: gemini-3.5-flash · gemini-3.5-flash-lite · gemini-3.6-flash
              gemini-3.7-flash · gemini-3.8-flash
   RPD > 500 인 모델이 나오면 AI_MODEL_* 5종과 AI_RPD_LIMIT_FLASH_LITE 만 바꾼다 (코드 변경 없음)
   ※ 추정 금지. 3.1.1절 절차로 실측하고 3.1절 표에 기록한다
   ※ 비-Lite `gemini-3.1-flash`는 카탈로그에 존재하지 않는다 — 이 ID를 쓰지 말 것
2) 평가 품질이 문제라면 모델이 아니라 프롬프트의 척도 기준을 더 조작적으로 다시 쓴다
   (02_ai_architecture.md 4.2.2절 — 2패스 평가자가 이미 모델 손실을 구조로 보상하고 있다)
3) 예산이 열리면 02_ai_architecture.md 4.3절 매핑(Claude)으로 전환한다
```

**정원이 모자랄 때의 1차 대응은 모델이 아니라 BYOK 유도입니다**(D28) — BYOK 세션은
하루 12건 정원 밖이고 개수 제한이 없습니다(`02_ai_architecture.md` 8.3.0절).
...
| **`AI_RPD_LIMIT_FLASH_LITE`** (활성 버킷 1종) | **실측값**(3.1절) | **Preview 키 기준의 실측값** — 운영값을 그대로 쓰면 원장이 없는 여력을 약속합니다 | 설정 안 함(fail-open) |
| **`AI_RPD_LIMIT_FLASH` / `_PRO`** | 비워 둠(휴면, D34) | 비워 둠 | 비워 둠 |
| **`AI_QUOTA_GATE_ENABLED`** | `true` | `true` | `false` 권장 — 로컬에서 게이트에 막히면 개발이 멈춥니다 |
| **`AI_QUOTA_RESET_TIMEZONE`** | 실측값 | 운영과 **같은 값** | 〃 |
| **`AI_RESERVE_*` 3종** | 기본값(**34/0/0** — D34) | 운영과 같은 값 | 〃 |
...
### 3.1 선택 모델의 무료 티어 한도 — **2026-09-11 실측 완료 (계정별 값이므로 계정이 바뀌면 다시 재야 합니다)**
...
**2026-09-11 실측 완료 — 이 계정 기준입니다**(`02_ai_architecture.md` 4.2.0절).
결제 미연동 무료 키로 https://aistudio.google.com/rate-limit 에서 직접 읽었습니다.
**다른 계정·다른 시점에서는 3.1.1절 절차로 다시 재야 합니다.**

| 모델 | RPM | TPM | **RPD** | 판정 | 확인일 | 출처 |
|---|---|---|---|---|---|---|
| **`gemini-3.1-flash-lite`** | **15** | **250,000** | **500** | **유일하게 쓸 수 있습니다 — 5개 역할 전부 이 모델** | **2026-09-11** | AI Studio 대시보드 |
| `gemini-3-flash` | 5 | 250,000 | 20 | 세션 1개도 못 돌립니다 | 2026-09-11 | 〃 |
| `gemini-2.5-flash-lite` | 10 | 250,000 | 20 | 〃 | 2026-09-11 | 〃 |
| `gemini-2.5-flash` | 5 | 250,000 | 20 | 〃 | 2026-09-11 | 〃 |
| `gemini-2.5-pro` | 0 | 0 | **0** | **무료 티어에서 사용 불가** | 2026-09-11 | 〃 |
| `gemini-3.1-pro` | 0 | 0 | **0** | **사용 불가**(카탈로그상 `gemini-3.1-pro-preview`) | 2026-09-11 | 〃 |
| `gemini-2-flash` / `gemini-2-flash-lite` | 0 | 0 | 0 | 이 계정에서 사용 불가(공식 문서상 shut down) | 2026-09-11 | 〃 |

> **이 실측이 초안의 전제를 뒤집었습니다.** 초안은 "Pro가 가장 빡빡하고 Flash-Lite가 가장 여유롭다"는
> 계층 구조를 가정해 역할별로 모델을 나눴습니다. 실제로는 **Pro 계열에 무료 여력이 아예 없고**,
> `gemini-2.5-flash-lite`의 RPD 20은 **초안의 세션당 예약량 26보다도 작습니다** —
> 설계대로 배포했다면 첫 `reserve_session_quota()` 호출이 반드시 실패해 **하루 면접이 0건**이었을 것입니다.
>
> **`[확인 필요]` 아직 재지 않은 최신 Flash 계열**이 카탈로그에 있습니다 — `gemini-3.5-flash`,
> `gemini-3.5-flash-lite`, `gemini-3.6-flash`, `gemini-3.7-flash`, `gemini-3.8-flash`.
> **이 중 하나라도 RPD가 500을 넘으면 하루 정원이 곧바로 늘어납니다.** 추정하지 말고 같은 대시보드에서
> 실측해 이 표에 추가하세요. **비-Lite `gemini-3.1-flash`는 존재하지 않습니다**(카탈로그 확인 완료).
>
> **3자 출처의 수치를 근거로 쓰지 마세요.** 일부 문서는 `gemini-3.1-flash-lite`를
> 30 RPM / 1,000~1,500 RPD로 적고 있으나 **이 계정의 대시보드 실측치는 15 RPM / 500 RPD**이고,
> 설계는 **낮은 쪽(실측치)** 을 씁니다.

**대조할 우리 소비량**(`02_ai_architecture.md` 8.1절, 20턴 세션 1건 기준):

```
호출 27회 = planner 1 + interviewer ≤20 + summarizer ≤3 + evaluator 2(패스 A+B) + coach 1
입력 약 76,000 토큰 / 출력 약 9,000 토큰
전부 gemini-3.1-flash-lite 한 모델에 몰립니다 (활성 버킷 flash_lite 하나)
...
| 2 | 모델별 RPD | **`AI_RPD_LIMIT_FLASH_LITE`에 그대로 주입할 값**(1.2절). 이 값이 없으면 게이트는 fail-open입니다. 휴면 버킷 2종은 주입하지 않습니다 |
| 3 | **`gemini-3.1-flash-lite`의 RPD** | **하루 체험 정원 = `floor(유효한도 / AI_RESERVE_FLASH_LITE_PER_SESSION)`.** **유일한 활성 버킷이라 이 값 하나가 정원을 결정합니다**(D34 — 이 자리에 있던 `pro`는 무료 RPD가 0이라 예약 자체가 불가능합니다). 실측 500 기준 **`floor(floor(500 × 0.85) / 34) = floor(425 / 34) = 12세션/일`** 이며, **사용자당이 아니라 서비스 전체 합계입니다.** **BYOK 세션은 이 계산 밖이고 개수 제한이 없습니다**(D28) |
| 4 | TPM | 세션당 입력 **76K**가 몇 세션까지 동시에 들어가는지. 실측 250K TPM은 30분에 걸쳐 흩어지므로 여유롭습니다 — **RPD만이 진짜 벽입니다** |
...
2) 모델별 RPM / RPD / TPM을 읽어 위 표에 적는다. 확인일도 함께 적는다
   필수: gemini-3.1-flash-lite  ← 실제로 쓰는 유일한 모델
   함께 볼 것: 아직 재지 않은 최신 Flash 계열(gemini-3.5-flash · 3.5-flash-lite ·
              3.6-flash · 3.7-flash · 3.8-flash). RPD > 500 이면 정원이 늘어난다
   (Preview 환경 키를 분리했다면 D24 — 그 키의 계정도 같은 방식으로 확인한다)
3) RPD를 운영 환경변수에 넣는다
   AI_RPD_LIMIT_FLASH_LITE  (원값 그대로. 안전 마진은 코드가 뺀다)
   AI_RPD_LIMIT_FLASH / AI_RPD_LIMIT_PRO 는 휴면 버킷이므로 비워 둔다 (D34)
...
**정원이 모자랄 때의 조정 순서 (`02_ai_architecture.md` 4.2절 — D34로 전면 교체됨):**

> **⚠️ 초안의 "1) Evaluator를 `gemini-2.5-pro` → `gemini-2.5-flash`로 내린다"는 삭제됐습니다.**
> `flash`의 실측 RPD가 20, `pro`가 0이라 **그 레버는 성립하지 않습니다**(3.1절 표).
> 5개 역할이 이미 유일하게 쓸 수 있는 모델에 모여 있어 **내릴 곳이 없습니다.**

```
1) BYOK 유도를 앞세운다  ← 1순위. 모델을 건드리지 않는다
   BYOK 세션은 하루 12건 정원 밖이고 개수 제한이 없다 (D28, 02_ai_architecture.md 8.3.0절)
   여력 소진 시 문구는 "내일 오세요"가 아니라 "키를 연결하면 지금 바로 시작할 수 있어요"

2) 미실측 최신 Flash 계열을 실측해 RPD > 500 인 모델을 찾는다  ← 정원을 늘리는 유일한 레버
   찾으면 AI_MODEL_* 5종 + AI_RPD_LIMIT_FLASH_LITE 만 교체한다 (코드 변경 없음)
   ※ 추정 금지. 3.1.1절 절차로 실측하고 3.1절 표에 기록한다

3) 세션당 예약량(AI_RESERVE_FLASH_LITE_PER_SESSION)의 재시도 여유 7을 줄인다
   34 → 30 이면 정원이 12 → 14 로 는다. 대신 재시도 여유가 얇아져
   pause_reason='rate_limited'(면접 도중의 벽)가 다시 나올 위험을 감수하는 것이다
   quota_overflow 관측(02_ai_architecture.md 8.3.9절)을 먼저 보고 결정한다

4) 그래도 부족하면 동시 세션 상한을 낮춘다 (4단계 폴백 사다리가 자동으로 paused 처리)
```

**평가 품질이 문제일 때의 레버는 위 목록에 없습니다.** 모델을 올릴 수 없으므로,
`02_ai_architecture.md` 4.2.2절이 정한 대로 **프롬프트의 척도 기준을 더 조작적으로 다시 쓰는 것**이
다음 레버입니다. 예산이 열리면 4.3절 매핑(Claude)으로 전환하고 2패스 평가자를 1패스로 되돌립니다.
...
- **1·2·3단계의 종료 전이는 예약 반납을 동반합니다**(`05_api_contract.md` 4.7.3절):
  `paused → completed`는 **평가·코치 몫 8을 남기고 나머지**, `paused → abandoned`와
  `evaluating → failed`는 전부. **D34로 버킷이 하나가 되면서 "`flash_lite`만 먼저 반납한다"는
  초안의 표현은 더 이상 맞지 않습니다** — 반납 대상은 버킷이 아니라 **남은 호출 몫**입니다
  (`02_ai_architecture.md` 8.3.1절·8.3.4절·8.3.6절). `05_api_contract.md` 4.7.3절이 원본입니다.
...
| # | 항목 | 미충족이면 |
|---|---|---|
| **P1** | **`AI_RPD_LIMIT_FLASH_LITE`를 AI Studio 대시보드에서 읽어 운영 환경변수에 주입했는가** (3.1.1절 절차 — **문서에는 없는 값이라 소유자만 읽을 수 있습니다.** 이 계정 실측값은 `500`, 휴면 버킷 2종은 비워 둡니다) | **게이트가 fail-open으로 동작합니다** — D27은 실질적으로 미적용이고 벽이 다시 면접 도중으로 돌아옵니다. 부팅 경고 로그로 확인하세요 |
...
[해소 2026-09-11 / D34] 3.1절 표 — 선택 모델들의 무료 티어 RPM / RPD / TPM을 실측했다.
            gemini-3.1-flash-lite = 15 RPM / 250K TPM / 500 RPD 가 유일하게 쓸 수 있는 값이고,
            gemini-2.5-* 계열은 RPD 20, Pro 계열은 RPD 0 이었다.
            → 초안의 "Pro가 가장 빡빡하고 Flash-Lite가 가장 여유롭다"는 전제가 틀렸다.
              5개 역할을 전부 gemini-3.1-flash-lite 로 모으고, 활성 버킷을 flash_lite 하나로
              줄였다(예약 34/0/0). 하루 체험 정원은 12세션이다.
            → 이 사실이 "수치를 환경변수로 빼고 미설정 시 fail-open" 설계를 검증했다.
              계정마다 다른 값을 코드에 박았다면 첫 배포부터 틀린 정원을 계산하고 있었을 것이다.
            → 값은 계정별이므로 계정·티어가 바뀌면 3.1.1절 절차로 다시 재야 한다.

[확인 필요 / 남음] 3.1절 표 — 아직 재지 않은 최신 Flash 계열(gemini-3.5-flash,
            gemini-3.5-flash-lite, gemini-3.6-flash, gemini-3.7-flash, gemini-3.8-flash).
            하나라도 RPD가 500을 넘으면 하루 정원(12세션)이 곧바로 늘어난다.
            추정하지 말고 같은 대시보드에서 실측해 3.1절 표에 추가할 것.
