...
/**
 * 데모 동의 문구의 버전 (D35-3). **체험과 별개 버전입니다** — 문구가 다르므로 해시도 다르고,
 * 한 계정이 둘 다 갖는 일은 없습니다(익명 계정은 체험을 할 수 없습니다).
 */
export const CURRENT_DEMO_CONSENT_VERSION = "demo-1.0.0";

/**
 * 데모 동의 문구 (`01_product_spec.md` 6.6.3절 3번 = `06_ui_plan.md` 4.15절).
 *
 * **이력서 문장이 빠져 있습니다** — 데모의 이력서는 우리가 준비한 시드이고, 외부로 나가는 것은
 * **방문자의 답변뿐**이기 때문입니다. 체험 문구를 그대로 재사용하면 사용자가 하지 않은 일
 * (자기 이력서 업로드)에 동의시키는 셈이 됩니다.
 */
export const DEMO_CONSENT_TEXT = `데모 면접을 시작하기 전에 확인해 주세요

데모 면접은 Google의 무료 AI 서비스로 진행됩니다. 답변하신 내용을 Google이 서비스 개선에 사용할 수 있고, 검토자가 읽을 수 있습니다. 이 기록은 24시간 뒤 자동으로 삭제됩니다.

위 내용을 확인했고 데모 면접 진행에 동의합니다`;
